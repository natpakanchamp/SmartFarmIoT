...
int readStableAnalog(int pin) {
  long sum = 0;
  int samples = 20; // อ่านซ้ำ 20 ครั้งแล้วเฉลี่ย
  for(int i=0; i<samples; i++) {
    sum += analogRead(pin);
    delay(2); // พักช่วงสั้นๆ ให้ ADC คืนตัว
  }
  return sum / samples;
}
...
void calculate(int currentHour, int currentMin){
  ...
  // อ่านค่าความชื้นดิน
  int rawSoil1 = readStableAnalog(SOIL_PIN);
  int rawSoil2 = readStableAnalog(SOIL_PIN2);
  int rawSoil3 = readStableAnalog(SOIL_PIN3);
  int rawSoil4 = readStableAnalog(SOIL_PIN4);
  int ASoil = (rawSoil1 + rawSoil2 + rawSoil3 + rawSoil4)/4;
  soilPercent = map(ASoil, AIR_VALUE, WATER_VALUE, 0, 100);
...
void loop() {
  ...
  if(millis() - lastCalcUpdate > 1000){
    ...
    if(getLocalTime(&timeinfo, 0)){
      ...
      float lux = lightMeter.readLightLevel();
      int rawSoil1 = analogRead(SOIL_PIN);
      int rawSoil2 = analogRead(SOIL_PIN2);
      int rawSoil3 = analogRead(SOIL_PIN3);
      int rawSoil4 = analogRead(SOIL_PIN4);
      int ASoil = (rawSoil1 + rawSoil2 + rawSoil3 + rawSoil4)/4;
      soilPercent = map(ASoil, AIR_VALUE, WATER_VALUE, 0, 100);
